...
<a href="yp_index"><button type="button" class="btn btn-danger btn-sm" >返回首页</button></a>
...
<div class="row">
    ...
    <div class="col-md-6">
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <li><a href="yp_cx?pn=1">首页</a></li>
                <!-- 判断是否有上一页 -->
                <c:if test="${pageInfo.hasPreviousPage }">
                    <li>
                        <a href="yp_cx?pn=${pageInfo.pageNum-1}"aria-label="Previous">
                ...
                <c:forEach items="${pageInfo.navigatepageNums }" var="page_num">
                    ...
                    <c:if test="${page_num != pageInfo.pageNum}">
                        <li><a href="yp_cx?pn=${page_num}">${page_num}</a></li>
                    </c:if>
                </c:forEach>
                <!-- 判断是否有下一页 -->
                <c:if test="${pageInfo.hasNextPage }">
                    <li>
                        <a href="yp_cx?pn=${pageInfo.pageNum+1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
                <li><a href="yp_cx?pn=${pageInfo.pages}">末页</a></li>
            </ul>
        </nav>
    </div>
</div>
